...
async def main():
    async with async_playwright() as p:
        ...
        with open("variants.csv", "w", newline="", encoding="utf-8") as f:
            ...
            for loc in locs:
                # Select Location
                await page.select_option('select[name="attribute_pa_location"]', loc['value'])
                for rval in rvals:
                    await page.select_option('select[name="attribute_pa_r-value"]', rval['value'])
                    for width in widths:
                        await page.select_option('select[name="attribute_pa_width"]', width['value'])
